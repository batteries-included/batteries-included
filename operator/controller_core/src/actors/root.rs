...
use crate::cs_client::{self, ControlServerClient};
use actix::prelude::*;

use super::{cluster_controller::ClusterControllerActor, core_messages::{CrdPresentMessage, DependenciesReadyMessage, StartMessage}, crd::CrdDependencyActor, namespace::NamespaceDependencyActor};
use tracing::debug;
...
impl Handler<CrdPresentMessage> for RootClusterActor {
    type Result = ();
    fn handle(&mut self, _msg: CrdPresentMessage, ctx: &mut Self::Context) -> Self::Result {

        // After the CRD is installed then the
        // kube cluster is ok enough to start installing what we need
        self.issue_system_sync(DependenciesReadyMessage, ctx);
    }
}