defmodule ControlServerWeb.Live.StatefulSetShow do
  ...
  def stateful_set_facts_section(%{} = assigns) do
    ~H"""
    <.badge>
  ...
  defp main_page(assigns) do
    ...
    <.page_header title={@name} back_link={~p"/kube/stateful_sets"}>
      <.stateful_set_facts_section resource={@resource} namespace={@namespace} />
    </.page_header>
      ...
      <.grid columns={[sm: 1, lg: 4]} class="lg:template-rows-2">
        ...
        <.panel title="Details" class="lg:col-span-3 lg:row-span-2">
          <.data_list id="status">
            ...
            <:item title="Current Revision">{Map.get(@status, "currentRevision", "")}</:item>
          </.data_list>
        </.panel>
      </.grid>
