defmodule ControlServerWeb.BackendServiceControllerTest do
  ...
  describe "index" do
    test "lists all backend_services", %{conn: conn} do
      conn = get(conn, ~p"/api/backend/services")
  ...
  describe "create service" do
    test "renders service when data is valid", %{conn: conn} do
      conn = post(conn, ~p"/api/backend/services", service: @create_attrs)
      assert %{"id" => id} = json_response(conn, 201)["data"]

      conn = get(conn, ~p"/api/backend/services/#{id}")
    ...
    test "renders errors when data is invalid", %{conn: conn} do
      conn = post(conn, ~p"/api/backend/services", service: @invalid_attrs)
  ...
  describe "update service" do
    ...
    test "renders service when data is valid", %{conn: conn, service: %Service{id: id} = service} do
      conn = put(conn, ~p"/api/backend/services/#{service}", service: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, service: service} do
      conn = put(conn, ~p"/api/backend/services/#{service}", service: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
  ...
  describe "delete service" do
    ...
    test "deletes chosen service", %{conn: conn, service: service} do
      conn = delete(conn, ~p"/api/backend/services/#{service}")
      assert response(conn, 204)

      assert_error_sent(404, fn ->
        get(conn, ~p"/api/backend/services/#{service}")
      end)
    end
  end
