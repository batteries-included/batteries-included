defmodule ControlServerWeb.MetricsControllerTest do
  use ControlServerWeb.ConnCase, async: false
  ...
  describe "GET /api/metrics" do
    test "returns prometheus format with correct content-type", %{conn: conn} do
      # Store test metrics in both control server stores
      Store.record_metric(
        ControlServerWeb.MetricsStore,
        "http_requests_total",
        42,
        %{"method" => "GET", "code" => "200"},
        :counter,
      ...
      # Trigger aggregation
      send(ControlServerWeb.MetricsStore, :aggregate_metrics)
      send(ControlServer.MetricsStore, :aggregate_metrics)
    ...
    test "handles store errors gracefully", %{conn: conn} do
      # Test graceful handling without stopping stores - empty response is valid
      conn = get(conn, ~p"/api/metrics")
