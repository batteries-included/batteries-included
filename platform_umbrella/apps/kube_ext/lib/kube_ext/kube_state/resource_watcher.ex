defmodule KubeExt.KubeState.ResourceWatcher do
    ...
    case K8s.Client.stream(conn, op) do
      {:ok, list_res} ->
        ...
        |> Enum.each(fn r ->
          ...
          # but
          KubeExt.KubeState.Runner.add(table_name, r, skip_broadcast: true)
        end)
  ...
  defp handle_watch_event("ADDED" = _event_type, object, state_table_name),
    do: KubeExt.KubeState.Runner.add(state_table_name, object)

  defp handle_watch_event("DELETED" = _event_type, object, state_table_name),
    do: KubeExt.KubeState.Runner.delete(state_table_name, object)

  defp handle_watch_event("MODIFIED" = _event_type, object, state_table_name),
    do: KubeExt.KubeState.Runner.update(state_table_name, object)
