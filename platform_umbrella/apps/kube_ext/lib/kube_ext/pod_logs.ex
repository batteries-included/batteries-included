defmodule KubeExt.PodLogs do
  ...
  - If you need single time access then you should look at `KubeExt.PodLogs.get_logs()`
  - If you want to get messages of each log line then look at `KubeExt.PodLogs.Worker`
  - If you want to get messages of log lines, and a starting set of recent
    logs (for example for UI display) then use `KubeExt.PodLogs.monitor()`
  """
  ...
  @doc """
  Monitor the log lines by spawning a `KubeExt.PodLogs.Worker` that will `send`
  each new log of a pod and get the latest set of lines.
  """
  def monitor(namespace, name, pid, opts \\ [tailLines: 50]) do
    {:ok, worker_pid} =
      KubeExt.PodLogs.Worker.start_link(namespace: namespace, name: name, target: pid)
