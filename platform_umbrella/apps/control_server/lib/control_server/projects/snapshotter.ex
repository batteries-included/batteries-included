defmodule ControlServer.Projects.Snapshoter do
  ...
  defp post_snapshot_proccessing(input, project) do
    input
  ...
  defp remove_postgres_cluster_passwords(input) do
    Map.update(input, :postgres_clusters, [], fn clusters ->
      case clusters do
        nil ->
          nil

        clusters ->
          # For each cluster remove the password versions
          # when they are re-imported they will be re-generated as new version
          #
          #  This is done to ensure that the passwords are not stored in the snapshot
          Enum.map(clusters, fn cluster ->
            Map.put(cluster, :password_versions, [])
          end)
      end
    end)
  end
  ...
  defp insert_multi(project, snapshot, context, opts) do
        ...
        |> Enum.reduce(multi, fn attrs, inner_multi ->
          ...
          changeset =
            apply(module, :changeset, [
              struct(module),
              prepare_for_insert(module, attrs, project, opts)
            ])
  ...
  defp update_multi(project, snapshot, context, opts) do
    Enum.reduce(Project.resource_types(), Multi.new(), fn type, multi ->
      module = module_by_resouce_type(type)
      ...
      if module do
        ...
        |> Enum.reduce(multi, fn attrs, inner_multi ->
          ...
          changeset =
            apply(module, :changeset, [existing, prepare_for_insert(module, attrs, project, opts)])
  ...
  defp prepare_for_insert(module, attrs, project, opts) do
    attrs
    |> ensure_map()
  ...
  defp ensure_map(attrs) when is_struct(attrs), do: Map.from_struct(attrs)

  defp ensure_map(attrs) when is_map(attrs), do: attrs
  defp ensure_map(_attrs), do: %{}

  defp remove_association(attrs, module) do
    asocs = get_associations(module)

    Enum.reduce(asocs, attrs, fn field, acc ->
  ...
  defp remove_autogenerated(attrs) do
    attrs
    |> Map.delete(:id)
    |> Map.delete(:__meta__)
  end

  defp maybe_change_virtual_size(attrs, module, opts) do
    case Keyword.get(opts, :virtual_size, nil) do
      nil ->
        attrs

      size ->
        try_set_virtual_size(attrs, module, size)
    end
  end

  defp maybe_remove_password_versions(attrs, Cluster) do
    Map.delete(attrs, :password_versions)
  end

  defp maybe_remove_password_versions(attrs, _module) do
    attrs
  end
