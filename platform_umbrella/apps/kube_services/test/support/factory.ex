defmodule KubeServices.Factory do
  ...
  def with_conditions(%{"conditions" => _} = status, conditions),
    do: %{status | "conditions" => conditions}

  def with_conditions(%{"status" => %{"conditions" => _} = status} = pod, conditions),
    do: %{pod | "status" => Map.put(status, "conditions", conditions)}

  def with_status(%{"status" => _, "type" => _} = condition, status),
    do: %{condition | "status" => status}

  def with_status(%{"status" => _} = pod, status), do: %{pod | "status" => status}

  def with_name(%{"name" => _} = metadata, name), do: %{metadata | "name" => name}

  def with_name(%{"metadata" => %{} = metadata} = pod, name),
    do: %{pod | "metadata" => with_name(metadata, name)}

  def with_namespace(%{"namespace" => _} = metadata, namespace),
    do: %{metadata | "namespace" => namespace}
