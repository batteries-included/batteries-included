defmodule CommonCore.Util.PolymorphicTypeTest do
  ...
  test "can create valid changeset" do
    ...
    assert %AdvancedConfig{non_default: "something", defaulted: nil, defaulted_override: nil, type: :advanced} = config
  end
  ...
  test "can override default field in config" do
    ...
    assert %AdvancedConfig{non_default: "set", defaulted: nil, defaulted_override: "overridden!", type: :advanced} =
             config
  end
  ...
  test "cast/2: handles unmapped data gracefully" do
    ...
    assert {:ok, %BasicConfig{non_default: nil, type: :basic}} =
             PolymorphicType.cast(basic_config(:struct, :empty), %{mappings: [], field: :config})

    # maps need mapping so we know which `cast` to call
    assert {:error, [config: "no matching type in mappings"]} =
             PolymorphicType.cast(basic_config(:map, :all), %{mappings: [], field: :config})
  end
  ...
  test "load/3: handles unmapped data gracefully" do
    ...
    assert :error = PolymorphicType.load(basic_config(:map, :all), &Function.identity/1, %{mappings: []})
    assert :error = PolymorphicType.load(advanced_config(:map, :empty), &Function.identity/1, %{mappings: []})
    assert :error = PolymorphicType.load(advanced_config(:map, :all), &Function.identity/1, %{mappings: []})
  end
