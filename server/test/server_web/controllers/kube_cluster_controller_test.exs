defmodule ServerWeb.KubeClusterControllerTest do
  ...
  describe "update kube_cluster" do
    ...
    test "renders kube_cluster when data is valid", %{conn: conn, kube_cluster: %KubeCluster{id: id} = kube_cluster} do
      conn = put(conn, Routes.kube_cluster_path(conn, :update, kube_cluster), kube_cluster: @update_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, kube_cluster: kube_cluster} do
      conn = put(conn, Routes.kube_cluster_path(conn, :update, kube_cluster), kube_cluster: @invalid_attrs)
      assert json_response(conn, 422)["errors"] != %{}
    end
  end
