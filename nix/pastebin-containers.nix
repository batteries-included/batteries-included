...

{
  perSystem = { self', pkgs, ... }:
    let
      ...
      contents = [ pkgs.bash pkgs.coreutils pkgs.bind pkgs.cacert pkgs.dockerTools.caCertificates ];
      Entrypoint =
      ...
      pastebin = self'.packages.pastebin;
      pastebin-static = self'.packages.pastebin-static;
    in
    {
      ...
      packages.pastebin-container = pkgs.dockerTools.buildLayeredImage {
        name = "pastebin";
        tag = taggedVersion;
        contents = [ pastebin pastebin-static ] ++ contents;
        config = {
          ExposedPorts = { "8080/tcp" = { }; };
          # Put the working dir in the pack
          WorkingDir = "${pastebin-static}/";
          Cmd = [ "${pastebin}/bin/pastebin" "." ];
          inherit Entrypoint;
        };
      };
    };
