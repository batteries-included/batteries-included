...
var wgConfTemplate = template.Must(template.New("wgconf").Parse(`# AUTOGENERATED
[Interface]
...
func (g *gatewayConfig) buildCloudConfig() (*cloudConfig, error) {
	...
	err := wgConfTemplate.Execute(buf, struct {
		Port        int
		GatewayCidr string
	}{g.port, g.gatewayCidrBlock.String()})
	if err != nil {
		return nil, err
	}
