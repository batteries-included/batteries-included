...
func (g *gateway) withConfig(cfg auto.ConfigMap) error {
	volumeSize, err := strconv.Atoi(cfg["cluster:gatewayVolumeSize"].Value)
	if err != nil {
		return err
	}
	...
	g.vpcCidrBlock = cfg["cluster:vpcCidrBlock"].Value
	g.gatewayCidrBlock = cfg["cluster:gatewayCidrBlock"].Value
	g.generateKey = cfg["cluster:gatewayGenerateKey"].Value == "true"
	g.instanceType = cfg["cluster:gatewayInstanceType"].Value
	g.volumeType = cfg["cluster:gatewayVolumeType"].Value
...
func (g *gateway) securityGroup(ctx *pulumi.Context) error {
	...
	rules := map[string]struct {
		...
		"wg-all":   {port: pulumi.Int(51820), proto: pulumi.String("udp"), cidr: allCidr},
